...
INSERT INTO 
employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) 
VALUES (206,'Shreya','Kedar','Shreya.Kedar@sqltutorial.org','825.123.8580','1997-06-07',2,12000.00,101,11);
...
SELECT first_name, last_name, salary AS new_salary FROM employees;
...
FROM ( SELECT DISTINCT salary FROM employees
			ORDER BY salary DESC
			OFFSET 1 ROWS) AS Second_Highest_Salary;
...
--- NOTE - SQL Server  do not support column order change directly. The column will be added at the end.
...
WHERE salary > (
					...
					FROM ( SELECT AVG(salary) AS avg_dept_salary
					FROM employees
...
SELECT * FROM employees
WHERE hire_date NOT BETWEEN '1989-01-01' AND '2000-12-31';
...
WHERE NOT EXISTS (SELECT *  FROM dependents d WHERE d.employee_id = e.employee_id);
...
--- 2. SQL LEFT JOIN clause 
---------------------------------

--- A. Query to the country names of US, UK, and China 
...
SELECT c.*, l.location_id FROM countries c
LEFT JOIN locations l ON c.country_id = l.city
...
--- A. The manager_id column specifies the manager of an employee. Write a query  statement to  joins the employees table to itself to query the information of who reports to whom.
...
--- B. The president does not have any manager. In the employees table, the manager_id of the row that contains the president is NULL.

SELECT 
 e.first_name AS employee,
 m.first_name AS manager
FROM employees e
LEFT JOIN employees m ON e.manager_id = m.employee_id --- LEFT JOIN SHOWS THE NULL VALUES

----------------------------------------------------------------------------------------------------------------------------------

--- 4. SQL FULL OUTER JOIN clause
...
VALUES (1, 'Wholesale'), (2, 'Retail'), (3, 'eCommerce'), (4, 'TV Shopping');
...
SELECT 
	department_id, job_id, COUNT(*) AS employee_count
FROM employees
...
SELECT
	m.employee_id AS manager_id,
	m.first_name + ' ' + m.last_name AS manager_name,
	COUNT(e.employee_id) AS direct_report
FROM employees e
JOIN employees m ON e.manager_id = m.employee_id
GROUP BY m.employee_id, m.first_name, m.last_name
...
--- A. Write a Query to combine the first name and last name of employees and dependentS.

SELECT first_name + ' ' + last_name AS employees_full_name
FROM employees

UNION 

SELECT first_name + ' ' + last_name AS dependents_full_name
FROM dependents
...
--- A. Write a Query to  Applies the INTERSECT operator to the A and B tables and sorts the combined result set by the id column in descending order.
...
SELECT 
  first_name,
  hire_date,
  (2000 - YEAR(hire_date)) AS years_completed,
...
--- A. Write a Query to update Sarah’s last name from  Bell to Lopez
...
--- A. Combine Above two queries using subquery inorder find all departments located at the location whose id is 1700 
...
WHERE salary = (SELECT MIN(salary) AS lowest_salary FROM employees)

													---OR---

SELECT d.department_id, d.department_name, e.salary FROM departments d
JOIN employees e ON d.department_id = e.department_id
WHERE salary = (SELECT MIN(salary) AS lowest_salary FROM employees)
...
WHERE salary > ( SELECT MAX(min_salary) 
					FROM(
						   SELECT MIN(salary) AS min_salary FROM employees
						   GROUP BY department_id) AS department_lowest_salary
						   );
...
WHERE salary >= (SELECT MAX(max_salary)
					FROM (
							...
							GROUP BY department_id) AS department_highest_salary
							);
...
SELECT department_id, 
						...
						WHERE e1.department_id = e2.department_id) AS avg_salary
FROM employees e1
GROUP BY department_id;
...
SELECT AVG(average_salary) AS average_of_average_salary
FROM(
